...
from purias_utils.error_modelling_torus.non_parametric_error_model.main import WorkingMemoryFullSwapModel
...
model_keys_across_set_sizes = [set(dg.all_metadata_inverted[separation_metadata_name].keys()) for N, dg in dataset_generator.data_generators.items()]
model_keys_across_set_sizes = list(set.union(*model_keys_across_set_sizes))
num_sep_models = len(model_keys_across_set_sizes)
#############################################
...
swap_models: Dict[Any, WorkingMemoryFullSwapModel] = torch.nn.ModuleDict({})


for model_key in model_keys_across_set_sizes:

    new_swap_model, D, delta_dimensions = setup_model_whole(
        ...
        error_emissions_keys=None,
        device=device,
    )

    swap_models[str(model_key)] = new_swap_model



opt = torch.optim.Adam(swap_models.parameters(), lr = args.lr)
#############################################



if args.drop_metadata_values is not None:
    for mdk in args.drop_metadata_values:
        assert mdk in model_keys_across_set_sizes, mdk
        dataset_generator.drop_data_by_metadata_value(metadata_selection_key=separation_metadata_name, metadata_selection_value=mdk)
        swap_models.pop(mdk)
        model_keys_across_set_sizes.remove(mdk)
        num_models -= 1
...
T = int(args.num_training_examples * num_set_sizes * steps_per_epoch * len(model_keys_across_set_sizes))

logging_frequency = args.logging_frequency if args.logging_frequency is not None else int(T / 4)
testing_frequency = args.testing_frequency if args.testing_frequency is not None else int(T / 4)

timer = LoopTimer(T)
print('No early stopper!')


training_step_per_model_key = {N: {mdk: [] for mdk in model_keys_across_set_sizes} for N in all_set_sizes}
all_average_llh_losses_per_set_size = np.zeros([T, num_models, num_set_sizes, num_sep_models])
all_average_elbos_per_set_size = np.zeros([T, num_models, num_set_sizes, num_sep_models])
all_kl_losses_per_set_size = np.zeros([T, num_models, num_set_sizes, num_sep_models])
all_dist_losses_per_set_size = np.zeros([T, num_models, num_set_sizes, num_sep_models])
...
recent_component_priors_per_mdlkey = {mdl: {N: {} for N in all_set_sizes} for mdl in model_keys_across_set_sizes}

all_pi_u_tildes = np.zeros([T, num_models, num_set_sizes, num_sep_models]) * np.nan
all_pi_1_tildes = np.zeros([T, num_models, num_set_sizes, num_sep_models]) * np.nan

if D > 0:
    all_inverse_ells = np.zeros([T, num_models, num_set_sizes, num_sep_models, D])
    all_scalers = np.zeros([T, num_models, num_set_sizes, num_sep_models])

else:
    all_pi_s_tilde_means = np.zeros([T, num_models, num_set_sizes, num_sep_models])
    all_pi_s_tilde_stds = np.zeros([T, num_models, num_set_sizes, num_sep_models])


scalar_plot_x_axis = np.arange(1, T+1)[None].repeat(num_models, 0)


num_emissions_parameters = swap_models[str(model_key)].generative_model.error_emissions.emission_parameter(all_set_sizes[0]).shape[-1]
all_emissions_parameters = np.zeros([T, num_models, num_set_sizes, num_sep_models, num_emissions_parameters])
